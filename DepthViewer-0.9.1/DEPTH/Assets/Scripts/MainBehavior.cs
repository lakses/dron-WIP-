...
public class MainBehavior : MonoBehaviour {
	...
	private float _frameCheckTimer = 0f;
    ...
    /* Циклическая обработка frame.jpg каждую секунду */
    if (_frameCheckTimer <= 0f) {
        string framePath = "C:/dron/frame.jpg";
        if (File.Exists(framePath)) {
            // Проверяем время последнего изменения файла
            DateTime lastWriteTime = File.GetLastWriteTime(framePath);
            if (lastWriteTime > _lastFrameProcessTime) {
                Debug.Log($"Обнаружен обновленный frame.jpg, отправка на обработку...");
                SelectFile(framePath);
                _lastFrameProcessTime = lastWriteTime;
            }
        }
        _frameCheckTimer = 0.5f; // Проверять каждую секунду
    }
    else {
        _frameCheckTimer -= Time.deltaTime;
    }
